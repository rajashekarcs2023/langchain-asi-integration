...
This script patches LangChain's callback system to handle tuple-formatted messages
used by advanced features like bind_tools and with_structured_output.
"""
...
# Patch LangChain's get_buffer_string function to handle tuple-formatted messages
def patched_get_buffer_string(message: Union[BaseMessage, str, Tuple]) -> str:
    """Get buffer string for message."""
    ...
    if isinstance(message, tuple):
        ...
        # Handle ('role', 'content') format
            ...
            # If it's not a recognized tuple format, convert to string
            return str(message)
    
    # Use the original function for other message types
...
async def test_patched_advanced_features():
    """Test if our patch has fixed the issues with advanced features."""
    print("=" * 80)
    print("TESTING PATCHED ADVANCED FEATURES")
    print("=" * 80)
...
if __name__ == "__main__":
    asyncio.run(test_patched_advanced_features())
